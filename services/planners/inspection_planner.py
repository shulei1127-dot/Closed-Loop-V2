        ...
        for item in normalized_records:
            ...
            task_plans.append(
                TaskPlanDTO(
                    ...
                    skip_reason=None
                    if eligible
                    else "不满足 customer_name 存在、service_type 含巡检、executor_name=舒磊、inspection_done=true、且工单链接或工单ID存在",
                    ...
                    planned_payload={
                        ...
                        "report_status_hint": data.get("report_status_hint") or "pending_report_match",
                        "report_word_file": data.get("report_word_file"),
                        "inspection_sync_state": data.get("inspection_sync_state"),
                    },
                )
            )
        return task_plans
